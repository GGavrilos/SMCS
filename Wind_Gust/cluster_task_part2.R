...
library(NlcOptim) # solnl: minimizing function under condition
library(stats)
library(lubridate)
source("functions.R")
...
for (t in 2:n){
  ...
  for (i in 1:K){  
    #confun=function(x){return(list(ceq=x[i],c=NULL))}
    upb = rep(C/2,K)
    ...
    #rej[i]= (solnl(X=starting_point,objfun=func,confun=confun,lb=lob,ub=upb,tolFun=1e-03,tolCon=1e-03)$fn>1/alpha)
